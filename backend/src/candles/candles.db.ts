...
export function insertCandles(db: CandlesDb, rows: Candle | Candle[]): void {
    ...
    const stmt = db.prepare(
        `INSERT OR IGNORE INTO Candle (t, o, h, l, c, v, 'to')
...
export function getLastCandle(db: CandlesDb): Candle | null {
    const row = db
        .prepare(`SELECT t, o, h, l, c, v, 'to' FROM Candle ORDER BY t DESC LIMIT 1`)
...
export function getFirstCandle(db: CandlesDb): Candle | null {
    const row = db
        .prepare(`SELECT t, o, h, l, c, v, 'to' FROM Candle ORDER BY t ASC LIMIT 1`)
        .get() as Candle | undefined;
    return row ?? null;
}
...
export function getLastNCandles(db: CandlesDb, n: number): Candle[] {
    if (n <= 0) return [];
    return db
        .prepare(`SELECT t, o, h, l, c, v, 'to' FROM Candle ORDER BY t DESC LIMIT ?`)
        .all(n) as Candle[];
}
...
export function getAllCandles(db: CandlesDb): Candle[] {
    return db
        .prepare(`SELECT t, o, h, l, c, v, 'to' FROM Candle ORDER BY t ASC`)
        .all() as Candle[];
}
